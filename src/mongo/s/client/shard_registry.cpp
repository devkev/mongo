...
void ShardRegistry::init(ServiceContext* service) {
    ...
    auto lookupFn = [this](OperationContext* opCtx,
        ...
        auto [returnData, returnTopologyTime, removedShards] =
            ...
            // Check if we need to refresh from the configsvrs.
            if (timeInStore.topologyTime >= cachedData.getTime().topologyTime ||
                ...
                return {*cachedData, timeInStore.topologyTime, {}};
            }
        }();
        ...
        for (const auto& latestConnString : latestConnStrings) {
            ...
            auto [newData, updatedShard] = ShardRegistryData::createFromExisting(
                returnData, latestConnString.second, _shardFactory.get());
            LOGV2_DEBUG(4620258, 1, "after createFromExisting", "newData"_attr = newData.toBSON());
            returnData = newData;

            // FIXME: don't even bother returning updatedShard anymore
            if (updatedShard) {
                invariant(!updatedShard->isConfig());
            }
        }
        ...
        // First successful lookup means that registry is up.
...
void ShardRegistry::updateReplSetHosts(const ConnectionString& newConnString) {
    ...
    if (auto shard = _configShardData.findByRSName(newConnString.getSetName())) {
        ...
        LOGV2_DEBUG(4620268, 1, "after createFromExisting", "newData"_attr = newData.toBSON());
        invariant(updatedShard && updatedShard->isConfig());
    ...
    if (status.getStatus() == ErrorCodes::ShutdownInProgress) {
        LOGV2_DEBUG(
            4620202,
            1,
            "Cant schedule ShardRegistry reload for RSM update, executor shutdown in progress");
        return;
    }
...
void ShardRegistry::toBSON(OperationContext* opCtx, BSONObjBuilder* result) const {
    BSONObjBuilder map;
...
bool ShardRegistry::reload(OperationContext* opCtx) {
    ...
    LOGV2_DEBUG(4620271, 1, "forcing reload", "newForceReloadIncrement"_attr = value);
...
void ShardRegistry::clearEntries(OperationContext* opCtx) {
...
ShardRegistry::Cache::ValueHandle ShardRegistry::_getData(OperationContext* opCtx) {
    ...
    // Ensure the cache knows the time it should be aiming for.
    auto now = VectorClock::get(opCtx)->getTime();
    _cache->advanceTimeInStore(
        _kSingleton,
        {now.topologyTime().asTimestamp(), _rsmIncrement.load(), _forceReloadIncrement.load()});

    auto data = _cache->acquire(opCtx, _kSingleton, CacheCausalConsistency::kLatestKnown);
    invariant(data, "No ShardRegistryData found");
    return data;
}

ShardRegistry::Cache::ValueHandle ShardRegistry::_getCachedData() const {
    auto data = _cache->peekLatestCached(_kSingleton);
    invariant(data, "No ShardRegistryData found");
    return data;
}
