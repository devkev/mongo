...
public:
    ...
    /**
     * Acquires the latest value from the cache, or a nullptr ValueHandle if the key is not present
     * in the cache.
     *
     * Doesn't change the cache (eg. doesn't promote the key to the top of the LRU list).
     * Doesn't attempt to lookup, and so doesn't block.
     */
    ValueHandle peekLatestCached(const Key& key) const {
        return {_cache.peekLatestCached(key)};
    }
